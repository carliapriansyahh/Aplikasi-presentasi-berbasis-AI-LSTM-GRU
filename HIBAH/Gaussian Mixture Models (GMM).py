...
# Reshape data untuk GMM
X_train_reshaped = X_train.reshape(X_train.shape[0], -1)
X_test_reshaped = X_test.reshape(X_test.shape[0], -1)

# Melatih model GMM untuk klasifikasi 'right' dan 'left'
gmm_right = GaussianMixture(n_components=5, random_state=42)
gmm_right.fit(X_train_reshaped[y_train == 1])

gmm_left = GaussianMixture(n_components=5, random_state=42)
gmm_left.fit(X_train_reshaped[y_train == 0])

# Prediksi menggunakan model GMM
y_pred = []
for sample in X_test_reshaped:
    log_likelihood_right = gmm_right.score([sample])
    log_likelihood_left = gmm_left.score([sample])
    y_pred.append(1 if log_likelihood_right > log_likelihood_left else 0)
...
print(f"GMM Model Accuracy: {accuracy:.2f}")
print(f"GMM Model Precision: {precision:.2f}")
print(f"GMM Model Recall: {recall:.2f}")